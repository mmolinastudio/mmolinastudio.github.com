...
;( function( window ) {
	...
	grid3D.prototype._init = function() {
		...
		this.grid = this.gridWrap.querySelector( 'ul.grid' );
		// main grid items
		...
		// content
		this.contentEl = this.el.querySelector( 'div.portfolio-content' );
		// content items
	...
	grid3D.prototype._hideContent = function() {
		var self = this,
			contentItem = this.el.querySelector( 'div.portfolio-content > .show' ),
	...
	*/
	...
	grid3D.prototype._resizePlaceholder = function() {
		...
		if( this.placeholder ) {
			...
			this.placeholder.style.left = Number( -1 * ( this.grid.offsetLeft + this.grid.offsetParent.offsetLeft - scrollX() ) ) + 'px';
			this.placeholder.style.top = Number( -1 * ( this.grid.offsetTop + this.grid.offsetParent.offsetTop - scrollY() ) ) + 'px';
